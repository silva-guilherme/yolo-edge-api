...
def parse_args():
    ...
    p.add_argument("--min-per-class", type=int, default=30,
                   help="Minimo de instancias por classe no split de treino")
    return p.parse_args()
...
def main():
    ...
    print(f"\n{'='*55}")
    print(f" Inspecao do Dataset: {Path(args.dataset).parent.name}")
    print(f"{'='*55}")
    ...
    for split in ["train", "valid", "test"]:
        ...
        if not labels_dir.exists():
            print(f"  [{split}] AVISO: diretorio nao encontrado")
            continue
        ...
        print(f"\n  [{split.upper()}]  {imgs} imagens  |  {total} anotacoes  |  {missing} sem label")
